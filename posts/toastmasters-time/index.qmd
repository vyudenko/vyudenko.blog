...
```{r}
sex_freq <- table(ifelse(is.na(data_tm$Sex), "NA", ifelse(data_tm$Sex == 0, "Female", "Male")))
...
```{r}
density_plot <- ggplot(data_tm, aes(x=Time)) +
...
box_plot <- ggplot(data_tm, aes(x=Time)) +
  geom_boxplot(fill = "lightblue", color = "black") +  # Customize the box plot appearance
  scale_x_continuous(breaks = seq(30, max(data_tm$Time, na.rm = TRUE), by = 30)) +
  geom_vline(aes(xintercept=120), color="red", linetype="dotted", size=1) +
  geom_vline(aes(xintercept=90), color="yellow", linetype="dotted", size=1) +
  geom_vline(aes(xintercept=60), color="green", linetype="dotted", size=1) +
  annotate("rect", xmin=120, xmax=Inf, ymin=-Inf, ymax=Inf, alpha=0.1, fill="red") +
...
```{r}
ggplot() +
  geom_density(data = filter(data_tm, Sex == 1), aes(x = Time, fill = "Women"), alpha = 0.5) +
  geom_density(data = filter(data_tm, Sex == 0), aes(x = Time, fill = "Men"), alpha = 0.5) +
  scale_x_continuous(breaks = seq(30, max(data_tm$Time, na.rm = TRUE), by = 30)) +
...
```{r echo=FALSE}
mod1 <- lm(Time ~ Sex + tt_participant_sex_prop, data = data_tm)
mod11 <- lm(tt_time ~ Sex + tt_participant_sex_prop, data = data_tm)
...
```{r}
ggplot() +
  geom_density(data = filter(data_tm, ON_OFF == 1), aes(x = Time, fill = "In-person"), alpha = 0.5) +
  geom_density(data = filter(data_tm, ON_OFF == 0), aes(x = Time, fill = "Online"), alpha = 0.5) +
  scale_x_continuous(breaks = seq(30, max(data_tm$Time, na.rm = TRUE), by = 30)) +
  geom_vline(aes(xintercept = 120), color = "red", linetype = "dashed", size = 1) +
  annotate("rect", xmin = 120, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "red") +
...
```{r}
mod2 <- lm(Time ~ ON_OFF, data = data_tm)
mod22 <- lm(tt_time ~ ON_OFF, data = data_tm)
...
data_tm %>% 
  ...
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_x_continuous(breaks = seq(0, max(data_tm$tt_time, na.rm = TRUE), by = 300)) +
...
```{r}
mod_q3 <- lm(Time ~ Year, data = data_tm)
mod_q31 <- lm(tt_time ~ Year, data = data_tm)
...
data_tm %>% 
  ...
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_x_continuous(breaks = seq(0, max(data_tm$tt_time, na.rm = TRUE), by = 500)) +
...
data_tm %>%
  ...
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_x_continuous(
    breaks = seq(0, max(data_tm$tt_participant_sex_prop, na.rm = TRUE), by = 1),
...
ggplot() +
  geom_histogram(data = filter(data_tm, TT == "1"), aes(x = Time), fill = "blue", alpha = 0.5) +
  geom_vline(aes(xintercept=120), color="red", linetype="dashed", size=1) +
  geom_vline(aes(xintercept=90), color="yellow", linetype="dashed", size=1) +
  geom_vline(aes(xintercept=60), color="green", linetype="dashed", size=1) +
...
-   Time does affect whether your speech is successful or not. In situations where you have limited time, you better take as much time as possible, but not a second more!
