...
description: |
  A study of timing of Toastmasters speeches.
...
In Toastmasters you are always told to mind the time, there is a special role dedicated specifically to that. You, me, everyone has a limited time and we can't afford to waste it. When someone is wasting our time talking, you can't help but with they just shut up.

That being said, there are cases when you find yourself clinging to the arms of your chair thinking "just keep talking".

But in the end you ought to obey the time limits - basic courtesy. But is there more to it than just respect? Does the time of one's speech has any effect on it's effectiveness?

As a proxy for this we will turn to Toastmasters International, whose meetings have an impromptu session, where each participant has 2 minutes to give a speech. After the last participant has concluded everyone proceeds to voting for the best speaker.

We assume that questions and their difficulty as well as other contextual factors are ceteris paribus. The only variable of interest is time.
...
Toastmasters are more organized and formal compared to many speaking clubs. Many of its members are middle-aged with higher education[^1]. These people value their time and that is why among the evaluators there is a Timer (Ta-da!). Timer's responsibility is to keep track of time and show colored signs to a speaker. Without going into much detail, green is an early warning, yellow - soon the time will be up, red - time is up, finish your speech. Timer also tracks the time of each speaker and presents Timer Report - sine qua non[^2] of this article.

</div>

[^1]: <https://www.toastmasters.org/resources/statistics-and-data-hub>
...
[^3]: Special thanks goes to Chamber and their Timer reports, without you this project would've taken much-much longer.

<div>

## Note on Timer reports

Timer tracks everyone's time, as can be seen on the reports. Here we will be only looking at **TT** session times. Reports usually have the following information useful for our analysis:
...
[^4]: There are many reports that don't include TT winner or times of some speakers. Moreover some speaker volunteer two or more times, which complicates analysis.

In total there 180 unique meetings in the dataset. Our sample is representative, as the number of men and women[^5] is almost equal.

[^5]: the gender of the speaker was assigned post hoc, based on the photos of the meeting, common sense and a little bit of prejudice :)
...
generate_plot <- function(distribution) {
  ...
  if (distribution == "uniform") {
    ...
    y <- dnorm(x, mean = 95, sd = 10)
    y[x >= 80 & x <= 110] <- dunif(x[x >= 80 & x <= 110], min = 80, max = 110)
    title <- "C"
...
In this part we are going to try and answer some of the hardest philosophical questions, most pertinent questions starting with...[^7]

[^7]: https://www.hsph.harvard.edu/news/features/do-women-talk-more-than-men/
...
Strangely enough, if in TT session you have more men - then TT session will be shorter by 585 seconds (\~10 mins). Why?
...
No, women actually speak a little less than men (at least in our sample)
:::
:::
...
Since the strat of covid TT session times have increased by 160 secs (\~1.5 speakers).

However this could be due to many other factors, like how many prepared speeches there were, how long the meeting is overal. This being said, not having to go to in-person meetings allows for longer meetings (!)

Doesn't look like it either. We can conclude that as n -\> \inf speeches will follow a Normal distribution.
:::
:::

## Have we started speaking longer?
...
data %>% 
  ...
  labs(title = 'Lengths of TT sessions through from 2018 to 2022', x = "TT session length, seconds") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold", family = "Roboto"),
        axis.title.y = element_blank(),
        panel.grid.major = element_line(),
...
::: panel
[TT participants]{.panel-name}

```{r}
data %>% 
  filter(Club %in% c("Chamber", "Dniprohills", "Arttalkers", "Dniprohills/Arttalkers", "KOMA", "Changemakers", "Svoya Rubashka")) %>% 
  ggplot(aes(x = tt_participants, y = Club, fill = Club)) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_x_continuous(breaks = seq(0, max(data$tt_participants, na.rm = TRUE), by = 4)) +
  labs(title = 'Number of participants') +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold", family = "Roboto"),
        panel.grid.major = element_line(),
        plot.subtitle = element_text(hjust = 0.5, size = 8, face = "italic", family = "Roboto"),
        axis.text.x = element_text(hjust = 0.5, family = "Roboto"),
        axis.title = element_blank(), 
        plot.caption = element_text(face = "bold", family = "Roboto"),
        legend.position = "none")
```
...
Flowchart made with ggflowchart [@ggflowchart]
...
#| message: false
require("checkdown")
check_question(c("Being on time"), options = c("Eye contact", "Being on time", "Structure", "Being liked bt the audience", "Evoking emotion", "Knowing your audience"), type = "radio", right = "Of course!", wrong = "Yes, but no...", random_answer_order = TRUE)
```
...
-   [Women are more likely to win, when there are few women participating in TT](#heading3);
...
::: panel
[Probit \| Logit]{.panel-name}

```{r}
# plot data
plot(x = data_logit$speaker_order, 
     y = data_logit$TT,
     main = "Probit and Logit Models Model of the Probability of Winning TT, Given Speaker Order",
     xlab = "Speaker Order",
...
text(17, 0.9, cex = 0.8, "Win")
text(1, 0.1, cex= 0.8, "Lose")

# add estimated regression line of Probit and Logit models
x <- seq(1, 18, 1)
y_probit11 <- predict(mod11, list(speaker_order = x), type = "response")
y_logit10 <- predict(mod10, list(speaker_order = x), type = "response")

lines(x, y_probit11, lwd = 3, col = "blue")
lines(x, y_logit10, lwd = 3, col = "black", lty = 2)
...
Waiting and speaking later during TT session does indeed increases the probability of winning by a whopping 0.09%!

This would be rather upsetting, yet this is a great news, because you can't expect to win over the audience by just waiting and giving a killer speech at the end of TT session. But one thing is clear as day here - the only people who don't win are those that don't try!
...
st <- "In TM those who speak for longer than 150 seconds
          ...
          clubs in this sample such a practice is not present."
...
Since we have a dozen of TT winners who spoke over 150 seconds, we will remove them from our regression.
:::

::: panel
[Regressions]{.panel-name}

```{r, code_folding=TRUE}
mod5 <- glm(TT ~ Time, data = data_logit, family=binomial(link="logit"))
mod55 <- glm(TT ~ Time, data = data_logit, family=binomial(link="probit"))
mod6 <- glm(TT ~ Time, data = filter(data_logit, Time < 150), family=binomial(link="logit"))
mod66 <- glm(TT ~ Time, data = filter(data_logit, Time < 150), family=binomial(link="probit"))
stargazer(mod5, mod6,
...
plot(x = data_logit$Time, 
     y = data_logit$TT,
     main = "Probit and Logit Models Model of the Probability of Winning TT, Given the Time",
...
When you have someones attention you have to be mindful of time. You are not given their attention indefinitely, you have to use this opportunity exhaustively making every second count. Speak fast and blurt out your thought and they will not undersatnd it fully. Speak too much and they switch off and feel like you can't properly articulate your thoughts. You have to finish by the time they lose focus!

## Hypothesis 3: {#heading3}

**Women are more likely to win, when there are few women participating in TT**

::: panelset
::: panel
[Male / Female]{.panel-name}

```{r}
ggplot(data_logit, aes(x = tt_participant_sex_prop, y = TT)) +
  geom_point() +
...
```{r, code_folding=TRUE}
mod8 <- glm(TT ~ Sex + tt_participant_sex_prop, data = filter(data_logit, tt_participant_sex_prop > 0.5), family=binomial(link="logit"))
stargazer(mod8,
          covariate.labels = c("Speaker's Gender", "Proportion of Genders"),
          header=FALSE, 
          title="Regression results", 
          type='text', 
          digits=3,
...
datatable(data1, options = list(pageLength = 5))
```
:::
...
Giving a successful speech isn't about whether you made it in time, it is about being able to clearly articulating an idea.

We have experimentally p

To study this further we would need to look at the content of the speech, audience demographics and so many more things..

famous speeches[^9]

[^9]: <https://youtube.com/playlist?list=PLosaC3gb0kGCnvkRvp8aOF4OIuGaqDHG8>

one safe rule of thumb is to speak for between 90-100% of your allowed time [^10]

[^10]: <http://sixminutes.dlugan.com/presentation-timing/>
