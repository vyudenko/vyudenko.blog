...
data_long1 <- data_long %>%
  mutate(activity = case_when(
    activity %in% c("Japanese", "Reading", "Study", "Planning") ~ "Study",
    activity %in% c("Transportation", "Misc") ~ "Misc",
...
# dt_fit <- tsibble(df_filtered_sleep) %>%
#   select(-activity) %>%
#   model(arima110 = ARIMA(hour ~ pdq(1,1,0),greedy=F), 
#         arima013 = ARIMA(hour ~ pdq(0,1,3),greedy=F),
#         arima212 = ARIMA(hour ~ pdq(2,1,2),greedy=F))
# 
# glance(dt_fit) %>% arrange(AIC) %>% select(.model:BIC)
# 
# dt_fit %>%
#   select(arima212) %>%
#   gg_tsresiduals()
...
#   features(.innov, ljung_box, lag = 10, dof = 5) #10 - 2 - 2 - 1 = lag - p - q - k
# 
# dt_forecast <- tsibble(df_filtered_sleep) %>%
#   model(arima212 = ARIMA(activity ~ pdq(2,1,2),greedy=F)) # we need this for the simulation to work 
# 
# sim <- dt_forecast %>% generate(h = 30, times = 5, bootstrap = TRUE)
# head(sim,6)
...
#Code taken and adapted from Garrick Aden's blog post [Visualizing mytime](https://www.garrickadenbuie.com/blog/visualizing-my-time/).
```
